...
void forward(int NP, int rang) {
  ...
  printf("Avant tous les send\n");
    ...
    for (int j = 0; j < size_y; j++) {
      ...
      for (int i = 0 ; i< size_x; i++) 
       {
          if(rang!=0 && i==rang*(g_size_x/NP) && j==0 && t>1)
          {
              MPI_Recv(&UPHY(t,0,0)+(rang*(g_size_x/NP)-1)*size_y,size_y,MPI_DOUBLE,rang-1,TAG_LAST_ROW,MPI_COMM_WORLD,&status); 
          }
          if(i==(rang+1)*(g_size_x/NP)-1 && t>1 && rang!=NP-1)
          {
            MPI_Wait(&HPHY_r,NULL);
          }
  ...
  for (t = 1; t < nb_steps; t++) 
  {
    ...
    if (rang!=0)     MPI_Isend(hFil+size_y,(g_size_x/NP)*g_size_y, MPI_DOUBLE, rang-1, TAG_FIRST_ROW, MPI_COMM_WORLD,&one);
    printf("Isend 1 rang 0 \n");
   if (rang!=0)      MPI_Irecv(hFil,(g_size_x/NP)*g_size_y, MPI_DOUBLE, rang-1, TAG_LAST_ROW, MPI_COMM_WORLD,&one);
    printf("Irecv 1 rang 0 \n");
    if (rang!=NP-1)  MPI_Irecv(hFil+size_y*(size_x-1),g_size_x/(NP)*g_size_y, MPI_DOUBLE, rang+1, TAG_FIRST_ROW, MPI_COMM_WORLD,&HPHY_r);
    printf("Irecv 1 rang NP-1\n");
    if (rang!=NP-1)  MPI_Isend(hFil+size_y*(size_x-2),g_size_x/(NP)*g_size_y, MPI_DOUBLE, rang+1, TAG_LAST_ROW, MPI_COMM_WORLD,&one);
    printf("Isend1 rang NP-1\n");
