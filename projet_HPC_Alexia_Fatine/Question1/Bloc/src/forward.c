...
void forward(int NP, int rang) {
  ...
  for (t = 1; t < nb_steps; t++) {
    ...
    if (rang!=0)     MPI_Send(hFil+size_y,size_x*size_y, MPI_DOUBLE, rang-1, TAG_FIRST_ROW, MPI_COMM_WORLD);
    printf("Send1\n");
    if (rang!=NP-1)  MPI_Recv(hFil+size_y*(size_x-1),size_x*size_y, MPI_DOUBLE, rang+1, TAG_FIRST_ROW, MPI_COMM_WORLD,&status);
    printf("Recv &\n");
    if (rang!=NP-1)  MPI_Send(hFil+size_y*(size_x-2),size_x*size_y, MPI_DOUBLE, rang+1, TAG_LAST_ROW, MPI_COMM_WORLD);
    printf("Send2\n");
    if (rang!=0)     MPI_Recv(hFil,size_x*size_y, MPI_DOUBLE, rang-1, TAG_LAST_ROW, MPI_COMM_WORLD,&status);
    printf("Recv2\n");
    ...
    MPI_Gather(hFil+g_size_y*(rang!=0), size_x*size_y, MPI_DOUBLE, g_hFil, size_x*size_y, MPI_DOUBLE, 0, MPI_COMM_WORLD);
